  ...
  return (
    <div
      className={SubframeUtils.twClassNames(
        "group/644741c6 flex cursor-pointer items-center gap-1.5 rounded-sm px-2.5 py-1.5 hover:bg-default-font/[0.05] active:bg-default-font/[0.09]",
        {
...
>(function FloatingToolbarRoot(
  ...
  return (
    <div
      className={SubframeUtils.twClassNames(
        "flex items-center gap-1 rounded-[22px] border border-solid border-[#ffffff33] px-2 py-1.5 shadow-glass-surface relative bg-panel/50 backdrop-blur-[28px] backdrop-saturate-[135%]",
        className
      )}
      ref={ref}
      {...otherProps}
    >
      ...
      {children ? (
        <div className="flex items-center gap-1 relative z-[1]">{children}</div>
      ) : null}
    </div>
  );
});
