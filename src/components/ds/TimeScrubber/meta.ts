...
export default defineMeta({
  ...
  description:
    "A time-range scrubber for metric panes: a quiet 48-bar backdrop (a fixed activity histogram in neutral-300) with the selected range shaded in brand-50, hairline brand-primary handles authored at 55% and 85% of the width, mono range tags pinned to those handles, and a 1H/1D/1W/1M range selector row. In this system brand ink is near-black over a near-white shade, so selection reads as weight rather than hue, per doctrine. The geometry is fixed — it is a static composition, not a draggable control — so consumers pair it with their own pointer handling and re-render it with new range labels.",
  usage:
  ...
  props: [
    ...
    },
    {
      ...
      description:
        "Tag pinned to the right of the 55% handle (e.g. a range-start timecode); renders nothing when omitted.",
    },
    {
      name: "rangeEnd",
      type: "React.ReactNode",
      description:
        "Tag pinned to the left of the 85% handle (e.g. a range-end timecode); renders nothing when omitted.",
    },
    {
      name: "className",
      type: "string",
