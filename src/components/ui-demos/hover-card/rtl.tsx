...
// Radix places content on a physical side; map the logical sides onto
// left/right based on the current direction.
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {physicalSides.map((side) => (
          <HoverCard key={side} openDelay={10} closeDelay={100}>
            <HoverCardTrigger render={<Button variant="outline">{t[side]}</Button>} />
            <HoverCardContent
              side={side}
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {logicalSides.map((side) => (
          <HoverCard key={side} openDelay={10} closeDelay={100}>
            <HoverCardTrigger render={<Button variant="outline">{t[side]}</Button>} />
            <HoverCardContent
              side={logicalSideToPhysical(side, dir)}
              className="flex w-64 flex-col gap-1"
              dir={dir}
            >
              <div className="font-semibold">{t.name}</div>
