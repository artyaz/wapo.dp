...
import { generateDisplacementMap, displacementMapKey } from "./displacement-map";
import { createLiquidGlass, type LiquidGlassHandle, type BackdropSpec } from "./webgl-refraction";

/**
 * GlassSurface — the universal liquid-glass primitive.
 *
 * One component, three implementations, identical semantics
 * ("swap implementation, never semantics"):
 *
 *  backdrop-filter  base tint + blur(40px) + saturate(150%)   (Regular anchor)
 *  svg-displacement Chromium: low-blur layer + bare url(#filter) layer whose
 *                   displacement map is generated at runtime for THIS
 *                   element's size and radius (R1/R2 fixed)
 ...
 *                   the live backdrop
 *
 * Every tier carries the canonical specular construction:
 *   - 20% white rim border
 *   - dual quad-stop sheen (160° key light + 340° counter-sheen)
 *   - shadow-glass-specular inset
 *
 * Never put url(#…) in a Tailwind arbitrary value — inline styles only.
 */
...
export function GlassSurface({
  ...
  React.useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      const scaleR = Math.round(base * CHROMATIC.r);
      const scaleG = Math.round(base * CHROMATIC.g);

      const geoKey = `${displacementMapKey({
        width: w,
        height: h,
        radius: r,
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      // (The negotiated strategy already probed WebGL, so this is rare.)
    }
    return () => {
      handle?.destroy();
  ...
  const baseLayerStyle: React.CSSProperties =
    activeStrategy === "backdrop-filter" || !glass
      ...
      : isSvg
      ? {
          // Low base blur under the displacement tier (review R4: 2–4px keeps
          // edge-bending crisp instead of displacing already-blurred content).
          backdropFilter: `blur(3px)`,
          WebkitBackdropFilter: `blur(3px)`,
        }
      : isWebgl && webglMode === "full"
      ? {
          // WebGL owns the material entirely — strip the CSS frost.
          backdropFilter: "none",
          WebkitBackdropFilter: "none",
          backgroundColor: "transparent",
        }
      : {
          // WebGL edge mode keeps the base material beneath the canvas.
          backdropFilter: `blur(${ramp.blur}px) saturate(${Math.round(
            ramp.saturate * 100
          )}%)`,
          WebkitBackdropFilter: `blur(${ramp.blur}px) saturate(${Math.round(
